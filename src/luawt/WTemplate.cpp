...
int luawt_WTemplate_conditionValue(lua_State* L) {
    ...
    return 1;
        ...
        return luaL_error(L, "Wrong arguments number for WTemplate.conditionValue: %d", argc);
    }
}
...
int luawt_WTemplate_bindEmpty(lua_State* L) {
    ...
    return 0;
        ...
        return luaL_error(L, "Wrong arguments number for WTemplate.bindEmpty: %d", argc);
    }
}
...
int luawt_WTemplate_getErrorText(lua_State* L) {
    int argc = lua_gettop(L) - 1;

    WTemplate* self = luawt_checkFromLua<WTemplate>(L, 1);
        if (argc == 0) {
    std::string result = self->getErrorText();
    lua_pushstring(L, result.c_str());
    return 1;

    } else {
        return luaL_error(L, "Wrong arguments number for WTemplate.getErrorText: %d", argc);
    }
}
...
int luawt_WTemplate_setCondition(lua_State* L) {
    ...
    return 0;
        ...
        return luaL_error(L, "Wrong arguments number for WTemplate.setCondition: %d", argc);
    }
}
